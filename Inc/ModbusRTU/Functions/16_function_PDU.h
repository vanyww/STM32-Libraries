...
#define __MODBUS_RTU_FUNCTION_16_MAX_REGISTERS_COUNT                                                             \
   ((__MODBUS_RTU_MAX_ADU_SIZE - (__MODBUS_RTU_ADU_COMMON_DATA_SIZE + sizeof(ModbusRTU_Function16_RequestData))) \
...
} ModbusRTU_Function16_RequestData;

typedef struct __attribute__((scalar_storage_order("big-endian"), packed))
{
   uint16_t DataRegisterAddress;
   uint16_t WrittenRegistersCount;
} ModbusRTU_Function16_ReplyData;

ModbusRTU_Status ModbusRTU_Process16FunctionRequest(__SDEVICE_HANDLE(ModbusRTU) *,
