...
static const uint8_t _CRC_HighPartTable[] =
...
static const uint8_t _CRC_LowPartTable[] =
...
uint16_t ModbusRTU_CRC16(void *data, size_t dataSize)
{
   uint8_t *bytes = data;

   uint8_t CRC_Low = UINT8_MAX;
   uint8_t CRC_High = UINT8_MAX;
   uint8_t index;

   while(dataSize-- > 0)
   {
      index = CRC_Low ^ *bytes++;
      CRC_Low = CRC_High ^ _CRC_HighPartTable[index];
      CRC_High = _CRC_LowPartTable[index];
   }
